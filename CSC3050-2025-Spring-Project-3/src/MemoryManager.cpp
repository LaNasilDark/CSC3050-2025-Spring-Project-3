...
bool MemoryManager::setLong(uint32_t addr, uint64_t val, uint32_t *cycles)
{
  ...
  // 仍然可以支持64位写入，但分为两个32位操作
  this->setInt(addr, val & 0xFFFFFFFF, cycles);
  this->setInt(addr + 4, (val >> 32) & 0xFFFFFFFF);
  return true;
}

uint64_t MemoryManager::getLong(uint32_t addr, uint32_t *cycles)
{
  // 仍然可以支持64位读取，但分为两个32位操作
  uint64_t low = this->getInt(addr, cycles);
  uint64_t high = this->getInt(addr + 4);
  return low | (high << 32);
}
